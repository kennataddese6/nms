                        ...
                        render={({ field, fieldState }) => (
                          <Field className="gap-1.5" data-invalid={fieldState.invalid}>
                            ...
                            <NativeSelect {...field} id="child-room">
                              ...
                              {rooms.map((r) => (
                                <option key={r.id} value={r.id}>{r.name} ({r.age_group})</option>
                              ))}
                            </NativeSelect>
                            {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                          </Field>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Field className="gap-1.5">
                          ...
                          <Textarea {...field} id="child-med" placeholder="Special medication details or asthma info" rows={2} />
                        </Field>
                      )}
                    />
                      ...
                      <Controller
                        ...
                        render={({ field, fieldState }) => (
                          <div className="space-y-1">
                            ...
                            {fieldState.invalid && <p className="text-[10px] text-destructive">{fieldState.error?.message}</p>}
                          </div>
                        )}
                      />
                      <Controller
                          ...
                          <td className="p-4 text-muted-foreground">
                            {child.child_parents?.map((cp: any, idx: number) => (
                              <div key={idx} className="text-xs">
                                {cp.parents?.profiles?.first_name} {cp.parents?.profiles?.last_name} ({cp.relationship})
                              </div>
                            )) || "None"}
                          </td>
                          <td className="p-4">
